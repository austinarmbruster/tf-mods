...
resource "aws_lb" "reverse-proxy-1" {
  count = "${replace(replace(length(var.azs), "/^[^1].*/", "0"), 
             "/^1$/", "1")}"
  ...
  subnet_mapping {
    ...
    subnet_id     = "${aws_subnet.reverse-proxy-public.0.id}"
  }
...
resource "aws_lb" "reverse-proxy-2" {
  count = "${replace(replace(length(var.azs), "/^[^2].*/", "0"), 
             "/^2$/", "1")}"
  ...
  subnet_mapping {
    ...
    subnet_id     = "${aws_subnet.reverse-proxy-public.1.id}"
  }
...
resource "aws_lb" "reverse-proxy-3" {
  count = "${replace(replace(length(var.azs), "/^[^3].*/", "0"), 
             "/^3$/", "1")}"
  ...
  subnet_mapping {
    ...
    subnet_id     = "${aws_subnet.reverse-proxy-public.2.id}"
  }
